...
export const HANDBOOK: HandbookTopic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      { h: 'The second look' },
      "When something got past you, the report offers a second look (S on a keyboard): the file opens again, read-only, with your pins where they were and every missed flag marked in amber. Click a mark and Lucien says why it mattered. Esc brings the report back, and the drawer keeps every file's last report.",
      { h: 'The clock' },
  ...
  },
  {
    ...
    body: [
      `Closing a file pays a few paper clips (${CLIPS.file}, plus ${CLIPS.firstSolve} for a first solve, ${CLIPS.sGrade} for an S, ${CLIPS.daily} for the daily, ${CLIPS.weekly} for the weekly); a rush pays ${CLIPS.rushPerThousand} per thousand points; a campaign file replayed more than three times pays ${CLIPS.wornFile}. Clips are spent at the Market on the phone: Lucien's coat and hat, the mug, the radio, the cat's fur and collar, an ornament for the corner, curtains. One item a day is a third off; holders of any tier get it for half.`,
      { h: 'The coin' },
